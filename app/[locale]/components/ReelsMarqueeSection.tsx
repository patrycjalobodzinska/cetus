...
export default async function ReelsMarqueeSection() {
  ...
  // Zapętlamy reele, żeby pas był ciągły (jak w Zaufali nam).
  const track = [...reels, ...reels, ...reels, ...reels];
  ...
  return (
    <section className="relative py-16 lg:py-24">
      ...
      {/* karuzela filmików na całą szerokość ekranu z rozmyciem po bokach */}
      <div className="group/reels relative left-1/2 right-1/2 -mx-[50vw] w-screen overflow-hidden py-2 [mask-image:linear-gradient(90deg,transparent,#000_6%,#000_94%,transparent)]">
        <div className="flex w-max animate-marquee-left gap-5 px-3 group-hover/reels:[animation-play-state:paused]">
          {track.map((reel, i) => (
            <div
              key={`${reel.url}-${i}`}
