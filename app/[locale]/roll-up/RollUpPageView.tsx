              ...
              <div
                ...
                style={{
                  clipPath: show
                    ? 'inset(0 0% 0 0)'
                    : 'inset(0 100% 0 0)',
                  transition: 'clip-path 1100ms cubic-bezier(0.7, 0, 0.3, 1), opacity 700ms ease-out, transform 1000ms ease-out',
                }}
              >
                <Image
                  src="/logocetus.png"
...
const POLYGON_CLIP = "polygon(0% 0px, 20px 0%, 95% 0%, 100% 20px, 100% 80%, 100% 100%, calc(100% - 20px) 100%, 5% 100%, 0% 80%)";
...
function ScenarioCard({ isOpen, onToggle, icon, title, subtitle, children }: ScenarioCardProps) {
  return (
    <div
        ...
        >
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs md:text-sm text-slate-500 mt-1 break-words">{subtitle}</p>
            </div>
            <ChevronDown
...
export default function RollUpPageView({ data }: { data: RollUpData }) {
  ...
  const [openSection, setOpenSection] = useState<'business' | 'career' | null>(null);
  ...
  const isCareerExternal = !!(
    data.careerCtaLink &&
    (data.careerCtaLink.startsWith('http://') || data.careerCtaLink.startsWith('https://'))
  );
  ...
  return (
    <>
      ...
      <section className="relative overflow-x-hidden max-w-[100vw] w-full flex flex-col items-center justify-start overflow-hidden">
        <div className="pt-[var(--page-top-offset)] w-full justify-center container mx-auto pb-6 md:pb-10 flex flex-col items-center relative px-4">
            ...
            {(data.heroTitle || data.heroTitleHighlight) && (
              ...
              >
                ...
                {data.heroTitleHighlight && (
                  <span className="text-blue-600">{data.heroTitleHighlight}</span>
                )}
              </h1>
            )}
      ...
      <section className="py-2 md:py-16 overflow-x-hidden">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          {(data.sectionTitle || data.sectionTitleHighlight || data.sectionSubtitle) && (
              ...
              {(data.sectionTitle || data.sectionTitleHighlight) && (
                ...
                >
                  ...
                  {data.sectionTitleHighlight && (
                    <span className="text-blue-600">{data.sectionTitleHighlight}</span>
                  )}
                </h2>
              )}
          ...
          <div className="space-y-4">
            <ScenarioCard
              ...
              icon={<Briefcase className="w-6 h-6 md:w-7 md:h-7 text-blue-600" aria-hidden="true" />}
            ...
            >
              ...
              {data.businessServices.length > 0 && (
                <ul className="space-y-2 mb-6">
                  {data.businessServices.map((service, idx) => {
                    const Icon = (service.icon && ICON_MAP[service.icon]) || Code2;
                    return (
                      <li key={idx}>
                        <Link
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {service.description && (
                              <p className="text-xs text-slate-500 hidden sm:block">{service.description}</p>
                            )}
                          </div>
                          <ArrowRight
              ...
              {(data.businessPrimaryButtonText || data.businessSecondaryButtonText) && (
                <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
                  {data.businessPrimaryButtonText && (
                    <Link href={resolveLink(data.businessPrimaryButtonLink, locale)} className="w-full sm:w-auto">
                      <StarGradientButton>
                  ...
                  {data.businessSecondaryButtonText && (
                    <Link
                      href={resolveLink(data.businessSecondaryButtonLink, locale)}
            ...
            <ScenarioCard
              ...
              icon={<GraduationCap className="w-6 h-6 md:w-7 md:h-7 text-blue-600" aria-hidden="true" />}
                ...
                <ul className="space-y-3 mb-6">
                  {data.careerBullets.map((bullet, idx) => (
                    <li key={idx} className="flex items-start gap-3">
                      ...
                      <span className="text-slate-700 text-sm md:text-base">{bullet}</span>
                    </li>
                  ))}
                </ul>
              )}
